...
侧炮与子机跟随武器等级成长，五组自动联动强化攻击形态；ECHO 后加入三种战术怪，十种精英进一步区分解法。见 [v6.3 设计](docs/design-v6.3.md)及[验证记录](docs/validation/v6.3.md)。排行榜服务配置仍见 [v6.2 部署说明](docs/design-v6.2.md)。
...
每波在 40／130／210／270／330 秒从对应两种模板中安排一只轻精英，共十种模板。精英不会暂停推进；到主首领节点仍未击败时停在节点并保留有限增援。击败轻精英提供一次强化和经验。所有主首领加入可读的位移、突进或原路回冲，动作结束时中心核心暴露；本体始终可伤，强构筑可以快速击破。PALISADE 拆臂提供两秒反击窗口，但双臂尽失后改用留有通路的双扇弹幕。
...
十四种普通敌人逐步引入并混编，不再按季别分池。保留弱点打断、镜盾护卫、贯穿炮破盾、重弹误伤、地雷和碎核爆破等战术关系。生命按预定推进时间增长，不根据玩家装备动态抬高。
...
ECHO 击破后的推进 100／125／165 秒依次引入裂隙追猎、折镜执灯、噤声指挥：侧翼锁路突袭、可拆双镜片激光，以及可以击破中继单位打断的错拍齐射。精英各自强调位移、拆部件、激光换路、旧路线回放、折返时序或内收弹环，五种远程模板不再套入冲刺。
...
v6.3 的翼炮随 Lv1–10 从 1 倍成长至 3.25 倍，每侧弹数在 I／III／V 为 1／2／3，均分总伤害；子机基础弹与新增子机主动模块按等级成长至 2.8 倍。不同子机模块还能提升基础子机弹，每种 +10%，最多计五种。贯通＋校准、翼炮＋校准／贯通、焦点灯＋束流棱台、共振集火＋追迹矩阵均自动联动，无需额外按键。
