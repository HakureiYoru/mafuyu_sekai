...
![主菜单](docs/validation/v4.1.0-menu.png)
...
| 贯穿炮 | 冲刺结束储存 4 秒，按 Q 手动发射 |
| 子机指令 | 准星指定目标后按 E，集火 4 秒、冷却 8 秒 |
| 炸弹 | Space，按下触发一次 |
| 暂停 / 继续 | Esc，失焦自动暂停 |
| 强化选择 | 点击或 1 / 2 / 3 |
...
冲刺冷却 2.6 秒，结束后获得一发储存 4 秒的贯穿炮：伤害 40、宽 88、长 2400，同时消除走廊中的敌弹，过热时也可用 Q 发射。左键不消耗这发储存。冲刺中按 Q 可预约一次；E 指令范围 560，分工索敌扩大至 720，无效目标不消耗技能。设置支持改键，Esc 与选卡 1/2/3 保留专用。

主炮只管理热量：连续射击约四秒过热，停火恢复；不再使用弹药。武装补给每单位减少冲刺和 E 恢复各一秒，两项均就绪时转换 20 XP。两档受伤保留武器等级，扣除当前升级需求 25% 的 XP，最多扣到零；已获得的共鸣等级不丢失。
...
第二季「镜界复奏」依次为镜界入口、幕门街区、中继回廊、复奏断层、裂核庭院、终章前线，各 75 秒。第二/第四段后必须击败 PALISADE / REPRISE，第六段结算选卡后进入 LACUNA 最终战。两位小 Boss 入场前分别将等级保底至 Lv5/Lv7，并各增加一台子机，已有三台时转为 60 XP；这些补给只在本局发放一次。击败各补 2 HP、1 炸弹及技能和冷却恢复。

普通敌人加入可利用的战术关系：射击发射器可打断蓄力；镜盾护卫可绕侧或用 Q 破盾；标记重弹能伤害其他普通怪；射爆地雷与碎核可炸伤附近敌群。PALISADE 左臂控制门阵，右臂控制定向扇束，拆臂取消对应未发动攻击。普通/困难敌群分别采用 10/4 秒与 12/3 秒的施压/回落节拍，最终符卡保持固定编排。
...
分光棱镜的 Shift+Q 取消副束，将首目标基础伤害提高至 52，后续目标仍为 40；轨道护刃会随 E 指令前出，转移途中不造成接触伤害。冷凝弹仓在热量达到 80 时降低 30，冷却十秒；擦弹降低热量并有限返还 E 冷却。首两次选项保证至少一个行为变化候选，最终战前排除当场无法发挥的纯清杂/拾取模块。
...
- 第二季开局满生命、清热、3 炸弹；两档已获得的等级均不因受伤降低。失败或刷新后从第二季第一段重新开始，模块及共鸣清空。
...
| `threat-director.ts` / `settings.ts` | 敌群角色、攻击预约和短程冲突筛查；兼容设置与改键 |
...
`node scripts/balance-v4.1.mjs` 运行三种种子、两档难度、低/中/高继承与三种构筑偏好的流程测量，并保留关闭 Q/E 的对照；正常生命与无敌输出分开记录。机器人不能替代真人难度评价，中等配置用于耗时校准，强构筑允许更快。`node scripts/verify-deployment.mjs` 检查线上版本与真实完成事件驱动的保存/刷新/第二季入口，使用独立浏览器档；`MAFUYU_DEPLOYMENT_URL` 可指定部署地址。
...
详见 [v4.1 设计](docs/design-v4.1.md) 与 [v4.1 验证](docs/validation/v4.1.0.md)。RTX 3060 Laptop 数据单列；核显 1080p 60 FPS 目标仍需对应设备实测。
...
每次功能/平衡变更更新唯一日志来源 `public/data/changelog.json`，最新在前，包含版本、日期、标题和 1–5 条重点。当前 **v4.1.0（2026-09-12）**。浏览器版本不包含 Electron 或触屏操作。
