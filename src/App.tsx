...
function Menu({ runtime, snapshot: s, openSettings, openChangelog }: { runtime: RuntimeControls; snapshot: HudSnapshot; openSettings: () => void; openChangelog: () => void }) {
  ...
  return <section className="menu-screen" aria-label="主菜单" style={{ '--scene-image': `url("${ASSET_URLS.bg}")` } as CSSProperties}>
    <div className="menu-simple">
      ...
      <p className="menu-tagline">笑梦来送快乐，学姐来送弹幕。</p>
      ...
      {s.difficulty === 'hard' && <p className="difficulty-note">学姐认真了 · 敌人更快 · 受到伤害 ×2</p>}
...
function Hud({ snapshot: s, runtime, openSettings }: { snapshot: HudSnapshot; runtime: RuntimeControls; openSettings: () => void }) {
  ...
  return <div className={`hud battle-hud ${s.phase !== 'playing' ? 'hud-inactive' : ''}`} aria-label="战斗状态">
    <div className="battle-top">
      <div className="stage-summary"><div><span className="status-dot" /><strong>{s.mode === 'endless' ? '无尽返场' : '笑梦串门'} · {s.difficulty === 'hard' ? '困难' : '普通'}</strong><span>{s.mode === 'endless' ? '∞' : `${Math.floor(s.progression / 360 * 100)}%`}</span></div><span className="stage-title">{s.stageName}{s.waveBlocked ? ' · 击败首领后继续' : ''}</span><Meter value={s.waveProgress} label="战役推进进度" /></div>
    ...
    <div className="battle-bottom">
      ...
      <div className="battle-comms"><button className="compact-comms-toggle" onClick={() => setCommsCollapsed(value => !value)} aria-expanded={!commsCollapsed}>{commsCollapsed ? '展开拌嘴' : '场边拌嘴'}<span>{s.comms?.speaker === 'EMU' ? '凤笑梦' : s.comms?.speaker === 'MAFUYU' ? '朝比奈真冬' : '场务'} {commsCollapsed ? '+' : '−'}</span></button>{!commsCollapsed && <p>{s.comms?.text || s.announcement || '笑梦正在想下一句 Wonderhoy。'}</p>}<div className="module-summary" aria-label="已装配模块">{s.modules.length} / 6 模块 · {s.evolutions.length} / 2 进化</div><div className="module-slots" aria-label="六个模块栏位">{Array.from({ length: 6 }, (_, i) => {
...
function UpgradeChoice({ snapshot: s, runtime }: { snapshot: HudSnapshot; runtime: RuntimeControls }) {
  ...
  return <Dialog title={s.choiceSource === 'boss' ? '首领奖励' : '选择强化'} eyebrow={`已装配 ${s.modules.length} / 6 · 已进化 ${s.evolutions.length} / 2`} className="upgrade-dialog">
    <p className="dialog-description">笑梦正在翻道具箱。战斗已暂停，按 1 / 2 / 3 或点击选择后继续。</p>
...
function Settings({ settings, setSettings, onClose }: { settings: GameSettings; setSettings: (settings: Partial<GameSettings>) => void; onClose: () => void }) {
  ...
  return <Dialog title="演出前，调一下" eyebrow="STAGE CHECK" onClose={onClose} className="settings-dialog">
...
function RunResults({ snapshot }: { snapshot: HudSnapshot }) {
  return <div className="run-results"><div><span>本局得分</span><strong>{snapshot.score.toLocaleString('en-US')}</strong></div><div><span>击破敌人</span><strong>{snapshot.kills}</strong></div><div><span>串门时长</span><strong>{formatTime(snapshot.elapsed)}</strong></div></div>;
}

function Changelog({ onClose }: { onClose: () => void }) {
  return <Dialog title="整活施工记录" eyebrow="RELEASE NOTES" onClose={onClose} className="changelog-dialog">
    <p className="dialog-description">笑梦追加了什么，学姐又要应付什么。</p>
    ...
    <button className="button button-secondary changelog-done" onClick={onClose}>看完了，回去整活</button>
  </Dialog>;
}
...
export default function App({ runtime }: { runtime: RuntimeControls }) {
  ...
  return <div className={`app phase-${snapshot.phase} ${snapshot.settings.reducedMotion ? 'reduce-motion' : ''}`}>
    ...
    {snapshot.phase === 'loading' && <section className="screen-overlay loading-screen" aria-label="加载游戏"><span className="loading-mark" aria-hidden="true">✦</span><div className="eyebrow">MAFUYU SEKAI</div><h1>笑梦正在搭台</h1><div className="loading-progress"><Meter value={snapshot.loading} label="资源加载进度" /><span>{Math.round(snapshot.loading * 100)}%</span></div><p>真冬：我好像还没同意。</p></section>}
    {snapshot.phase === 'upgrade' && <UpgradeChoice snapshot={snapshot} runtime={runtime} />}
    {!settingsOpen && snapshot.phase === 'paused' && <Dialog title="学姐，暂停一下！" eyebrow="WONDER… WAIT!" onClose={() => runtime.resume()}><p className="dialog-description">笑梦去喝口水，真冬终于清静了。战斗已暂停。</p><RunResults snapshot={snapshot} /><div className="dialog-actions"><button className="button button-primary" onClick={() => runtime.resume()}><Icon name="play" />继续游戏 <kbd>ESC</kbd></button><button className="button button-secondary" onClick={openSettings}><Icon name="settings" />体验设置</button><button className="text-button centered" onClick={() => runtime.returnToMenu()}>结束本局，返回主菜单</button></div><PausedModules snapshot={snapshot} /><ControlsGuide settings={snapshot.settings} compact /></Dialog>}
    {!settingsOpen && snapshot.phase === 'failed' && <Dialog title="Wonderhoy 卡壳了" eyebrow="WONDER… OOPS!" className="result-dialog failure-dialog"><div className="result-emblem" aria-hidden="true">✧</div><p className="dialog-description">笑梦被学姐礼貌送回起点。再次挑战从 Lv1 开始。</p><RunResults snapshot={snapshot} /><SaveNotice snapshot={snapshot} /><div className="result-detail"><span>{snapshot.stageName} · 推进 {Math.floor(snapshot.progression / 360 * 100)}%</span><span>最高纪录 {snapshot.bestScore.toLocaleString('en-US')}</span></div><div className="dialog-actions"><button className="button button-primary" onClick={() => runtime.restart()}><Icon name="restart" />再次挑战</button><button className="button button-secondary" onClick={() => runtime.returnToMenu()}>返回主菜单</button></div></Dialog>}
    {!settingsOpen && snapshot.phase === 'complete' && <Dialog title="Wonderhoy，整活成功！" eyebrow="ONE MORE WONDERHOY!" className="result-dialog complete-dialog"><div className="result-emblem" aria-hidden="true">✦</div><p className="dialog-description">真冬：「可以收工了吗？」笑梦：「还可以无尽返场！」继续无尽会保留本局构筑。</p><RunResults snapshot={snapshot} /><SaveNotice snapshot={snapshot} /><div className="dialog-actions"><button className="button button-secondary" onClick={() => runtime.continueEndless()}><Icon name="spark" />继续 · 无尽挑战<Icon name="arrow" /></button><button className="button button-secondary" onClick={() => runtime.restart()}><Icon name="restart" />重新开始</button><button className="text-button centered" onClick={() => runtime.returnToMenu()}>返回主菜单</button></div></Dialog>}
