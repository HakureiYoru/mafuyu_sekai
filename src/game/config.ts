...
export const ENEMIES: Record<EnemyType, { hp: number; speed: number; radius: number; color: number; label: string }> = {
  ...
  dasher: { hp: 14, speed: 155, radius: 36, color: 0xffcb69, label: '突进' },
  sniper: { hp: 18, speed: 60, radius: 36, color: 0xc99dff, label: '狙击' },
  sprayer: { hp: 32, speed: 45, radius: 70, color: 0x69caff, label: '扫射' },
  minelayer: { hp: 24, speed: 100, radius: 55, color: 0x95e4ab, label: '布雷' },
  ...
  shield: { hp: 26, speed: 125, radius: 38, color: 0xffd18c, label: '镜盾卫' },
  weaver: { hp: 24, speed: 90, radius: 38, color: 0xd2a3ff, label: '幕门织者' },
  returner: { hp: 16, speed: 140, radius: 32, color: 0xff9aad, label: '折返投手' },
  sampler: { hp: 22, speed: 115, radius: 34, color: 0xffba77, label: '轨迹采样者' },
  repairer: { hp: 24, speed: 110, radius: 34, color: 0xaed18c, label: '中继修复者' },
  carrier: { hp: 16, speed: 155, radius: 32, color: 0xff889c, label: '裂核载体' },
  palisade: { hp: 2000, speed: 200, radius: 88, color: 0xf1bd78, label: 'PALISADE / 门阵枢纽' },
  reprise: { hp: 3200, speed: 220, radius: 78, color: 0xd7a2ff, label: 'REPRISE / 复奏体' },
