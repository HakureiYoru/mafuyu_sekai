...
export const BALANCE = {
  ...
  ai: { maxCommitments: 3, commitGap: 0.28, mineSpacing: 110, mineSafeDistance: 150, hazards: 48 },
...
export const ENEMIES: Record<EnemyType, { hp: number; speed: number; radius: number; color: number; label: string }> = {
  ...
  sprayer: { hp: 32, speed: 45, radius: 70, color: 0x69caff, label: '失控扫射' },
  minelayer: { hp: 24, speed: 100, radius: 55, color: 0x95e4ab, label: '噤声布雷' },
  mine: { hp: 1, speed: 0, radius: 35, color: 0xff684f, label: '地雷' },
  boss: { hp: 700, speed: 30, radius: 160, color: 0xc7adff, label: 'MAFUYU / 空心微笑' },
  miniboss: { hp: 1600, speed: 240, radius: 64, color: 0xff956b, label: 'ECHO / 狂躁回声' },
  shield: { hp: 26, speed: 125, radius: 38, color: 0xffd18c, label: '空心盾卫' },
  weaver: { hp: 24, speed: 90, radius: 38, color: 0xd2a3ff, label: '封锁幕墙' },
  returner: { hp: 16, speed: 140, radius: 32, color: 0xff9aad, label: '折返投手' },
  sampler: { hp: 22, speed: 115, radius: 34, color: 0xffba77, label: '残影采样' },
  repairer: { hp: 24, speed: 110, radius: 34, color: 0xaed18c, label: '断线修复' },
  carrier: { hp: 16, speed: 155, radius: 32, color: 0xff889c, label: '崩裂载体' },
  palisade: { hp: 3600, speed: 200, radius: 88, color: 0xf1bd78, label: 'PALISADE / 噤声牢笼' },
  reprise: { hp: 6500, speed: 220, radius: 78, color: 0xd7a2ff, label: 'REPRISE / 回响不止' },
  arm: { hp: 300, speed: 0, radius: 24, color: 0xffc27c, label: '噤声发射臂' },
  node: { hp: 130, speed: 0, radius: 25, color: 0xc4a5ff, label: '冷光节点' },
  core: { hp: 4, speed: 0, radius: 12, color: 0xff9d87, label: '碎裂核心' },
};
