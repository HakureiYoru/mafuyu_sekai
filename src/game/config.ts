...
export const ENEMIES: Record<EnemyType, { hp: number; speed: number; radius: number; color: number; label: string }> = {
  ...
  miniboss: { hp: 1200, speed: 240, radius: 64, color: 0xff956b, label: 'ECHO / 狂躁回声' },
  ...
  palisade: { hp: 2000, speed: 200, radius: 88, color: 0xf1bd78, label: 'PALISADE / 噤声牢笼' },
  reprise: { hp: 3200, speed: 220, radius: 78, color: 0xd7a2ff, label: 'REPRISE / 回响不止' },
  arm: { hp: 100, speed: 0, radius: 24, color: 0xffc27c, label: '噤声发射臂' },
