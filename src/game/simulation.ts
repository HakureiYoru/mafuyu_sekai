...
import { createBuild, offerModules, chooseModule, buildDamageMultiplier, addResonanceXp } from './upgrades';
import { createSpellBrain, spellCardDefinition, advanceSpellCard, updateSpellBoss } from './spellcards';
import { createSeason2Brain, updateSeason2Ai, shieldDamageMultiplier, onSeason2Death, type Season2AiContext } from './season2-ai';
import type { AreaHazard, Bullet, CarryoverSnapshot, CombatEvent, Companion, Difficulty, Enemy, EnemyShotOptions, EnemyType, InputAction, ModuleId, Pickup, PickupType, Player, ProjectileMotionPhase, RunStartOptions, WorldState } from './types';
...
export class GameSimulation {
  ...
  continueEndless(): void {
    ...
    player.invincible = 2;
    player.dashTime = player.perfectWindow = player.shotCooldown = 0;
    this.clearInput();
    this.specialWindup = 0;
    this.specialTarget = null;
  ...
  private updatePlayer(input: InputAction, dt: number): void {
    ...
    if (this.has('magnet') && this.magnetTimer <= EPSILON) { this.magnetTimer = 10; this.magnetTime = 0.8; }
    if (this.has('reserveAmmo') && p.ammo < 20 && this.reserveTimer <= EPSILON) { p.ammo = Math.min(BALANCE.ammo.max, p.ammo + 30); this.reserveTimer = 10; }
    ...
    if (dashDelta > 0) {
      ...
      if (p.dashTime <= EPSILON) {
        ...
        if (this.has('vent') && this.ventTimer <= EPSILON) { p.heat = Math.max(0, p.heat - 25); this.ventTimer = 4; }
      }
    }
  ...
  private fireDashBeam(): void {
    ...
    for (let i = 0; i < Math.min(dash.targets, this.impacts.length); i++) {
      ...
      this.damageEnemy(this.impacts[i].enemy, dash.damage * buildDamageMultiplier(this.state.build), { x: p.x, y: p.y });
    }
    if (this.has('prism') && this.state.status === 'playing') for (const side of [-1, 1]) {
      ...
      for (const enemy of this.state.enemies) if (enemy.hp > 0 && !hitIds.has(enemy.id) && pointInBeam(enemy.x, enemy.y, enemy.radius, geometry)) {
        hitIds.add(enemy.id); this.damageEnemy(enemy, 12 * buildDamageMultiplier(this.state.build), { x: p.x, y: p.y });
      }
    }
  }
  ...
  private updateCompanions(dt: number): void {
    ...
    for (let i = 0; i < companions.length; i++) {
      ...
      if (this.has('orbitBlade')) for (const enemy of this.state.enemies) {
        ...
        this.damageEnemy(enemy, 4 * buildDamageMultiplier(this.state.build), { x: companion.x, y: companion.y });
      }
    }
    if (this.has('intercept') && this.interceptTimer <= EPSILON) {
      ...
      if (closest) { closest.life = 0; this.interceptTimer = 8; this.emit({ type: 'hit', x: closest.x, y: closest.y, color: 0x98efff, amount: 0 }); }
    }
  }
  ...
  private queueSpawn(): void {
    ...
    if (w.seasonId === 's2') {
      const stage = SEASONS.s2.stages[Math.min(w.wave - 1, 5)], pool = stage.enemyPool!;
      // First appearances receive a short isolated introduction before mixed groups.
      type = pool[Math.floor(this.random.next() * pool.length)];
      if (w.mode === 'story') {
  ...
  private season2Context(canCommit: (kind?: 'wall' | 'sample') => boolean = () => true): Season2AiContext {
  ...
  private updateHazards(dt: number): void {
    ...
    for (const hazard of w.hazards) {
      ...
      if (hazard.active && p.invincible <= EPSILON && hit) this.damagePlayer();
  ...
  private updateEnemies(dt: number): void {
    ...
    const canCommit = (e: Enemy, kind?: 'wall' | 'sample') => {
    ...
    for (let i = 0; i < count && w.status === 'playing'; i++) {
      ...
      if (e.season2) {
        updateSeason2Ai(e, dt, this.season2Context(kind => canCommit(e, kind)));
  ...
  private enemyShootAt(e: Enemy, x: number, y: number, angle: number, speed: number, radius: number, color: number, options?: EnemyShotOptions): void {
    ...
    if (bullet) {
      ...
      bullet.attackGroup = ['weaver', 'palisade', 'arm'].includes(e.type) ? 'wall' : undefined;
    ...
    for (let i = 0; i < bullets.length; i++) {
      ...
      if (!dead && b.owner === 'player') {
        ...
        for (const impact of this.impacts) {
          ...
          const enemy = impact.enemy, wasAlive = enemy.hp > 0, cardIndex = enemy.spell?.cardIndex;
          this.damageEnemy(enemy, b.damage, { x: b.prevX, y: b.prevY });
          if (this.state.status === 'playing' && enemy.spell?.cardIndex === cardIndex) this.applyBulletModules(b, enemy, wasAlive && enemy.hp <= 0);
          b.remainingHits--;
        ...
        if (segmentCircleHit(b.prevX - p.prevX, b.prevY - p.prevY, b.x - p.x, b.y - p.y, 0, 0, radius) !== null) {
          this.damagePlayer(b.damage); dead = true;
        } else if (this.has('graze') && !b.grazed && this.grazeCount < 3
          ...
          b.grazed = true; this.grazeCount++; p.ammo = Math.min(BALANCE.ammo.max, p.ammo + 1); p.heat = Math.max(0, p.heat - 1);
        }
      }
  ...
  private applyBulletModules(b: Bullet, enemy: Enemy, killed: boolean): void {
    ...
    if (b.kind === 'special' && this.has('chain')) {
      ...
      for (const target of targets) { b.hitIds.add(target.id); this.damageEnemy(target, 5 * multiplier, enemy); }
    }
  ...
  damageEnemy(enemy: Enemy, amount: number, source: { x: number; y: number } = this.state.player): void {
    if (this.state.status !== 'playing' || enemy.hp <= 0 || amount <= 0 || (enemy.disabledUntil ?? 0) > this.state.elapsed || enemy.spell?.stage === 'intro') return;
    const shield = shieldDamageMultiplier(enemy, source.x, source.y);
    const damage = (enemy.type === 'dasher' && enemy.state === 'charge' ? Math.ceil(amount * 1.3) : amount) * shield;
    ...
    if (enemy.type === 'node') {
      ...
      this.emit({ type: 'kill', x: enemy.x, y: enemy.y, targetId: enemy.id, enemyType: enemy.type, color: ENEMIES.node.color, amount: enemy.radius });
      return;
    }
    ...
    if (enemy.type === 'boss') {
      ...
      const p = this.state.player;
      p.hp = Math.min(p.maxHp, p.hp + 2); p.ammo = BALANCE.ammo.max; p.heat = p.heatLock = 0; p.overheated = false;
  ...
  private damagePlayer(damage: number = difficultyConfig(this.state.difficulty).damage): void {
    ...
    while (loss > p.xp && p.level > this.state.build.levelFloor) { loss -= p.xp; p.level--; p.xp = xpNeeded(p.level); }
    ...
    if (p.hp <= 0 && this.has('revive') && !this.reviveUsed) { this.reviveUsed = true; p.hp = 1; p.invincible = 0.8; }
